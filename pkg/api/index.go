...
func setIndexViewData(c *m.ReqContext) (*dtos.IndexViewData, error) {
	...
	if hasEditPermissionInFoldersQuery.Result {
		...
		if c.OrgRole == m.ROLE_ADMIN || c.OrgRole == m.ROLE_EDITOR {
			children = append(children, &dtos.NavLink{Text: "文件夹", SubTitle: "创建一个新文件夹来组织仪表板", Id: "folder", Icon: "gicon gicon-folder-new", Url: setting.AppSubUrl + "/dashboards/folder/new"})
		}

		children = append(children, &dtos.NavLink{Text: "导入", SubTitle: "从文件或Grafana.com导入仪表板", Id: "import", Icon: "gicon gicon-dashboard-import", Url: setting.AppSubUrl + "/dashboard/import"})
	...
	dashboardChildNavs := []*dtos.NavLink{
		{Text: "根目录", Id: "home", Url: setting.AppSubUrl + "/", Icon: "gicon gicon-home", HideFromTabs: true},
	...
	if c.IsSignedIn {
		...
		if !setting.DisableSignoutMenu {
			...
			profileNode.Children = append(profileNode.Children, &dtos.NavLink{
				Text: "退出登录", Id: "sign-out", Url: setting.AppSubUrl + "/logout", Icon: "fa fa-fw fa-sign-out", Target: "_self",
			})
		}
	...
	if setting.AlertingEnabled && (c.OrgRole == m.ROLE_ADMIN || c.OrgRole == m.ROLE_EDITOR) {
		alertChildNavs := []*dtos.NavLink{
			...
			{Text: "通知通道", Id: "channels", Url: setting.AppSubUrl + "/alerting/notifications", Icon: "gicon gicon-alert-notification-channel"},
		}
		...
		if c.IsGrafanaAdmin {
			cfgNode.Children = append(cfgNode.Children, &dtos.NavLink{
				...
				Children: []*dtos.NavLink{
					...
					{Text: "统计数据", Id: "server-stats", Url: setting.AppSubUrl + "/admin/stats", Icon: "fa fa-fw fa-bar-chart"},
